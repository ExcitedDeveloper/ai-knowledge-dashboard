...
const mockCreateEmbedding = createEmbedding as jest.MockedFunction<
  typeof createEmbedding
>;
  ...
  describe('handleSearch', () => {
    ...
    test('should return results sorted by similarity (descending)', async () => {
      ...
      const mockSelect = jest
        .fn()
        .mockResolvedValue({ data: mockDbFiles, error: null });
      (supabase.from as jest.Mock).mockReturnValue({
        select: mockSelect,
      });
    ...
    test('should create excerpts with highlighted query terms', async () => {
      ...
      const mockSelect = jest
        .fn()
        .mockResolvedValue({ data: [mockDbFile], error: null });
      (supabase.from as jest.Mock).mockReturnValue({
        select: mockSelect,
      });
    ...
    test('should handle files without embeddings', async () => {
      ...
      expect(
        result.find(
          (r: { filename: string }) => r.filename === 'no-embedding.pdf'
        )
      ).toBeUndefined();
    });
    ...
    test('should filter out results below 0.25 similarity threshold', async () => {
      ...
      const lowSimilarityFile = result.find(
        (r: { filename: string }) => r.filename === 'low-similarity.pdf'
      );
