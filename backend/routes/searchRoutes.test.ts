...
const mockHandleSearch = handleSearch as jest.MockedFunction<any>
...
describe('searchRoutes', () => {
  ...
  describe('GET /search', () => {
    test('should call handleSearch controller with correct parameters', async () => {
      mockHandleSearch.mockImplementation(async (req: any, res: any) => {
    ...
    test('should handle multiple query parameters with same name', async () => {
      ...
      const response = await request(app)
        .get('/search?query=first&query=second')
    ...
    test('should preserve request and response objects', async () => {
      ...
      expect(capturedReq.query).toEqual({ query: 'test' })
    })
  })
