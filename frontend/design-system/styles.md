...
```css
.promo-card {
  background: linear-gradient(135deg, #c04848 0%, #e6a04e 50%, #4a8fb8 100%);
}
...
```tsx
<Sidebar>
  <SidebarSection title="Workspace" defaultExpanded={true} collapsible={true}>
...
Remember: These aren't arbitrary rules—they're distilled wisdom from successful products serving millions of users. Use them as foundations, adapt them to your specific needs, but always understand _why_ they work before changing them.
